...
T3SPI::T3SPI(KINETISK_SPI_t * spi_used) {
...
void T3SPI::begin_MASTER() {
	setMCR(MASTER);
	setCTAR(CTAR_MODE0,8,SPI_MODE0,LSB_FIRST,SPI_CLOCK_DIV8);
...
void T3SPI::setFrameSize(uint8_t CTARn, uint8_t size) {
	stop();
	if (CTARn==0){
		SPIx->CTAR0 |= SPI_CTAR_FMSZ(size);}
	if (CTARn==1){
		SPIx->CTAR1 |= SPI_CTAR_FMSZ(size);}
	if (CTARn==2){
		SPIx->CTAR0 |= SPI_CTAR_FMSZ(size);}
	start();
}
...
void T3SPI::setMode(uint8_t CTARn, uint8_t dataMode) {
	stop();
	if (CTARn==0){
		SPIx->CTAR0 = SPIx->CTAR0 & ~(SPI_CTAR_CPOL | SPI_CTAR_CPHA) | dataMode << 25;}
	if (CTARn==1){
		SPIx->CTAR1 = SPIx->CTAR1 & ~(SPI_CTAR_CPOL | SPI_CTAR_CPHA) | dataMode << 25;}
	if (CTARn==2){
		SPIx->CTAR0 = SPIx->CTAR0 & ~(SPI_CTAR_CPOL | SPI_CTAR_CPHA) | dataMode << 25;}
	start();
}
...
void T3SPI::setBitOrder(bool CTARn, uint8_t bo) {
	...
	if (CTARn==0){
		if (bo == LSBFIRST) {
			SPIx->CTAR0 |= SPI_CTAR_LSBFE;}
		if (bo == MSBFIRST) {
			SPIx->CTAR0 &= ~SPI_CTAR_LSBFE;}}
	if (CTARn==1){
		if (bo == LSBFIRST) {
			SPIx->CTAR1 |= SPI_CTAR_LSBFE;}
		if (bo == MSBFIRST) {
			SPIx->CTAR1 &= ~SPI_CTAR_LSBFE;}}
	start();
}
...
void T3SPI::setClockDivider(bool CTARn, uint8_t cdiv) {
	stop();
	if (CTARn==0){
		SPIx->CTAR0 |= SPI_CTAR_DBR | SPI_CTAR_CSSCK(cdiv) | SPI_CTAR_BR(cdiv);}
	if (CTARn==1){
		SPIx->CTAR1 |= SPI_CTAR_DBR | SPI_CTAR_CSSCK(cdiv) | SPI_CTAR_BR(cdiv);}
	start();
}

void T3SPI::start() {
	SPIx->MCR &= ~SPI_MCR_HALT & ~SPI_MCR_MDIS;
}
...
// 	if (((SPIx->CTAR0 & 0x78000000)>>27)==7){
// 		bytesSent = (packetCT * length);}
// 	if (((SPIx->CTAR0 & 0x78000000)>>27)==15){
...
// 	for (unsigned int mask = 0x80000000; mask; mask >>= 1) {
// 		Serial.print(mask&SPIx->SR?'1':'0');}
// 	Serial.println();
// 	Serial.print("SPIx->RSER:       ");
// 	for (unsigned int mask = 0x80000000; mask; mask >>= 1) {
// 		Serial.print(mask&SPIx->RSER?'1':'0');}
// 	Serial.println();
// 	Serial.println();
