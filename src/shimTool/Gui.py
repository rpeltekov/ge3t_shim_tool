...
class Gui(QMainWindow):
    ...
    def initUI(self):
        ...
        if self.debugging:
            ...
            self.saveStateButton = addButtonConnectedToFunction(
                stateButtonLayout, "DEBUG ONLY: Save Current State", self.saveState
            )
            self.loadStateButton = addButtonConnectedToFunction(
                stateButtonLayout, "DEBUG ONLY: Load Latest State", self.loadState
            )
    ...
    def setupBasicTabLayout(self, layout: QBoxLayout):
        ...
        self.roiSliceIndexPack = addLabeledSliderAndEntry(imageLayout, "Slice Index (Int): ", self.updateROIImageDisplay)
        ...
        for i in range(3):
            ...
            self.roiPositionSliders[i] = addLabeledSlider(
                roiPositionSliders, f"Center {label[i]}", self.roiSliderGranularity
            )
    ...
    def setupExsiButtonsAndLog(self, layout: QBoxLayout):
        # create the buttons
        self.reconnectExsiButton = addButtonConnectedToFunction(
            layout, "Reconnect EXSI", self.shimTool.exsiInstance.connectExsi
        )
        self.doCalibrationScanButton = addButtonConnectedToFunction(
            layout, "Do Calibration Scan", self.doCalibrationScan
        )
    ...
    def setupManualShimLayout(self, layout: QBoxLayout):
        ...
        self.shimCalChannelsButton = addButtonConnectedToFunction(
            manualShimButtonsLayout,
            "Calibrate Shim Channels",
            self.shimTool.shimInstance.shimCalibrate,
        )
        self.shimZeroButton = addButtonConnectedToFunction(
            manualShimButtonsLayout,
            "Zero Shim Channels",
            self.shimTool.shimInstance.shimZero,
        )
        self.shimGetCurrentsButton = addButtonConnectedToFunction(
            manualShimButtonsLayout,
            "Get Shim Currents",
        ...
        self.shimManualChannelEntry = addEntryWithLabel(
            setChannelCurrentShimLayout,
            "Channel Index (Int): ",
            QIntValidator(0, self.shimTool.shimInstance.numLoops - 1),
        )
        self.shimManualCurrenEntry = addEntryWithLabel(
            setChannelCurrentShimLayout, "Current (A): ", QDoubleValidator(-2.4, 2.4, 2)
        )
        self.shimManualSetCurrentButton = addButtonConnectedToFunction(
            setChannelCurrentShimLayout, "Shim: Set Currents", self.shimSetManualCurrent
        )
        self.slowButtons += [
            self.shimCalChannelsButton,
            self.shimZeroButton,
            self.shimGetCurrentsButton,
            self.shimManualSetCurrentButton,
        ]
    ...
    def setupShimScanConfigurations(self, layout: QBoxLayout):
        ...
        self.shimGradientStrengthPack = addLabeledSliderAndEntry(calibrationStrengthView, "Gradient Cal (tick?): ", updateGradientCalStrength)
        updateSliderEntryLimits(
        ...
        self.shimCalCurrentPack = addLabeledSliderAndEntry(calibrationStrengthView, "Loop Cal (mA): ", updateLoopCalCurrent)
        updateSliderEntryLimits(
    ...
    def setupShimScanUI(self, layout):
        ...
        self.doOverwriteBackgroundButton = addButtonConnectedToFunction(
            settingsButtons, "Overwrite Background?", self.overwriteBackground
        )
        self.doOverwriteBackgroundButton.setSizePolicy(
            QSizePolicy(QSizePolicy.Policy.Minimum, QSizePolicy.Policy.Fixed)
        )
        self.doOverwriteBackgroundButton.setEnabled(
            False
        )  # this should become checkable when we have the first "actual shimmed background" completed
        ...
        self.setShimButton = addButtonConnectedToFunction(
            solutionAndSetCurrentLayout,
            "Apply Shim\nSolutions",
            self.setAllShimCurrents,
        )
        ...
        self.doBackgroundScansButton = addButtonConnectedToFunction(
            configLayout, "Obtain Field Map", self.doFieldmapScan
        )

        self.doLoopCalibrationScansButton = addButtonConnectedToFunction(
            boundingLayout, "Obtain Basis Field Maps", self.doLoopCalibrationScans
        )
        self.slowButtons += [
            self.recomputeCurrentsButton,
            self.setShimButton,
    ...
    def setWindowAndExamNumber(self, examNumber=None, patientName=None):
        ...
        self.guiWindowTitle = f"[ Shim Control GUI | EXAM: {examNumber or '!'} |" + f" Patient: {patientName or '!'} ]"
    ...
    def visualizeROI(self):
        ...
        if (
            not self.shimTool.ROI.enabled
            or self.roiVizButtonGroup.checkedId() == 0
            or not self.shimTool.obtainedBackground()
        ):
    ...
    def validateBasisInputs(self):
        ...
        # set the viewDataSlice to the desired slice from the whole 4D data set
        self.viewDataSlice[2] = self.shimTool.viewData[2][self.getBasisFunctionIndex()][
            :, self.getBasisSliceIndex()
        ]
        return True
    ...
    def requireAssetCalibration(func):
        ...
        def wrapper(self, *args, **kwargs):
            ...
            if not self.shimTool.assetCalibrationDone and not self.debugging:
                ...
                # Show a message to the user, reconnect exsi client.
                createMessageBox(
                    "Asset Calibration Scan Not Performed",
    ...
    def doGetAndSetROIImage(self):
        ...
        trigger.finished.connect(action)
        kickoff_thread(
            self.shimTool.getAndSetROIImage,
            args=(
                self.getROIBackgroundSelected() == 1,
                trigger,
            ),
        )
    ...
    def setAllShimCurrents(self):
        ...
        trigger.finished.connect(actionAndUpdate)
        kickoff_thread(
            self.shimTool.setAllShimCurrents,
            args=(
