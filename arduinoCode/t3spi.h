...
#define SPI_WRITE_8(c,CTARn, PCS, SPIx) \
	do { \
		while ((SPI0_SR & SPI_SR_TXCTR) >= 0x00004000); \
		SPIx->PUSHR = ((c)&0xff)   | SPI_PUSHR_CTAS(CTARn) | SPI_PUSHR_PCS(0x1f & PCS); \
	} while(0)


#define SPI_WRITE_16(w,CTARn, PCS, SPIx) \
	do { \
		while ((SPI0_SR & SPI_SR_TXCTR) >= 0x00004000); \
		SPIx->PUSHR = ((w)&0xffff) | SPI_PUSHR_CTAS(CTARn) | SPI_PUSHR_PCS(0x1f & PCS); \
	} while(0)


#define SPI_WAIT(SPIx) \
	while ((SPIx->SR & SPI_SR_TXCTR) != 0); \
...
public:
	...
	void setCTAR(bool CTARn, uint8_t size, uint8_t dataMode, uint8_t bo, uint8_t cdiv);
	void enableCS(uint8_t cs, bool activeState);
...
private:
	...
	void setBitOrder(bool CTARn, uint8_t bo);
	void setClockDivider(bool CTARn, uint8_t cdiv);
	void enablePins(uint8_t sck, uint8_t mosi, uint8_t miso, uint8_t cs, bool activeState);
	void enablePins_SLAVE(uint8_t sck, uint8_t mosi, uint8_t miso, uint8_t cs);
void setCS_ActiveLOW(uint32_t);
};
